...
public class RangeSearchVisualizer {
   ...
   * and inserts those points into a 2d-tree. Then, it performs range searches on
   * the axis-aligned rectangles dragged by the user in the standard drawing
   * window.
   * 
   */
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      if (StdDraw.isMousePressed() && !isDragging) {
        ...
        isDragging = true;
      } else if (StdDraw.isMousePressed() && isDragging) { // user is dragging a rectangle
        x1 = StdDraw.mouseX();
        y1 = StdDraw.mouseY();
      } else if (!StdDraw.isMousePressed() && isDragging) { // user stops dragging rectangle
        isDragging = false;
      }
