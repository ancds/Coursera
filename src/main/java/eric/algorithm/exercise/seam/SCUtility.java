...
public class SCUtility {
   ...
   * normalized by the maximum grayscale value (ignoring border pixels).
   */
  public static Picture doubleToPicture(double[][] grayValues) {
    ...
    for (int col = 0; col < width; col++) {
      for (int row = 0; row < height; row++) {
        ...
        picture.set(col, row,
            new Color(normalizedGrayValue, normalizedGrayValue, normalizedGrayValue));
      }
    }
  ...
  /**
   * This method is useful for debugging seams. It overlays red pixels over the
   * calculate seam. Due to the lack of a copy constructor, it also alters the
   * original picture.
   */
